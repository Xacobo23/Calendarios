...
def create_modules (n = 20):
    modules_instances = []

    for fp_name, courses in modules_by_fp.items():
        fp = FP.objects.filter(short_name=fp_name).first()

        if not fp:
            print(f'FP "{fp_name}" no encontrado, omitiendo...')
            continue
    ...
    Module.objects.bulk_create(modules_instances)
    print(f"{len(modules_instances)} módulos creados exitosamente.")
